      ...
      their number (should be fp.diag[I])	*****/
{
	...
	long long	DEP, len, rank, n;
	long long *xnum, xk, xbij, *comcoi, vj, *vec, **xvec;
	long long *FAiI, *Vvj, **Fvi, *scpvec, **Fxbase, **xbase, *comtri;
	scpcomb	com;
...
{
	FILE	*outfile;
	long long	i, j, dim, step, im, **C, nC, nH, found, try;
	long long	*orb, *oc, noc, *bad, nbad, *x;
	long long ***H, **Ggng;
...
/* x is the new base, i.e. x[i] is the index in V.v of the i-th base-vector */
	if ((x = (long long*)malloc(dim * sizeof(long long))) == 0)
		exit (1);
	if ((bad = (long long*)malloc(2*V.n * sizeof(long long))) == 0)
		exit (1);
...
/* a new generator has been found */
			{
				++G->ng[step];
				if ((G->g[step] = (long long***)realloc(G->g[step], G->ng[step] * sizeof(long long**))) == 0)
					exit (1);
				if ((G->g[step][G->ng[step]-1] = (long long**)malloc(dim * sizeof(long long*))) == 0)
					exit (1);
				for (i = 0; i < dim; ++i)
				{
					if ((G->g[step][G->ng[step]-1][i] = (long long*)malloc(dim * sizeof(long long))) == 0)
						exit (1);
				}
